...
(function () {
    ...
    angular
        ...
        .controller('AppAdminDialPlanController', AppAdminDialPlanController);

    AppAdminDialPlanController.$inject = ['$translate', '$state', 'cacheService', 'toastr', 'branchNumberClientService', 'dialPlanClientService', 'lodash', '$uibModal'];


    /** @ngInject */
    function AppAdminDialPlanController($translate, $state, cacheService, toastr, branchNumberClientService, dialPlanClientService, lodash, $uibModal) {
        ...
        function openForm(_item) {
            $state.go('admin_dialPlan_form', {branch: _item, state_back: 'admin_dialPlan'});
        }
        ...
        function deleteBranch(_item) {
            ...
            modalInstance.result
                .then(function (result) {
                    if (result) {
                        dialPlanClientService.removeClientDialPlan(vm.client._id, _item.branch)
                            .then(function (result) {
                                if(result.length >= 1){
                                    vm.branchList = result;
                                    vm.branchList_copy = angular.copy(result);
