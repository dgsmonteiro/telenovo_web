...
(function () {
    ...
    angular
        .module('app.admin.commands')
        .controller('AppAdminCommandsController', AppAdminCommandsController);

    AppAdminCommandsController.$inject = ['$translate', '$state', 'cacheService', 'toastr', 'generalClientService', 'lodash', '$uibModal'];


    /** @ngInject */
    function AppAdminCommandsController($translate, $state, cacheService, toastr, generalClientService, lodash, $uibModal) {

        /* jshint validthis: true */
        var vm = this;
        vm.client = null;
        vm.user = null;
        vm.filter_type = 'queueName';
        vm.filter_value = undefined;
        vm.executeFilter = executeFilter;
        vm.clearFilter = clearFilter;
        vm.getCommands = getCommands;
        ...
        function getCommands() {
            generalClientService.getListClientCommandsByClientId(vm.client._id)
        ...
        function openForm(_item) {
            $state.go('admin_commands_form', {command: _item, state_back: 'admin_commands'});
        }
        ...
        function deleteBranch(_item) {
            ...
            modalInstance.result
                .then(function (result) {
                    if (result) {
                        generalClientService.removeClientCommands(vm.client._id, _item.branch)
                            .then(function (result) {
                                if(result.length >= 1){
                                    vm.branchList = result;
                                    vm.branchList_copy = angular.copy(result);
