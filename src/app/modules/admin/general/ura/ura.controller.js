...
(function () {
    ...
    angular
        ...
        .controller('AppAdminUraController', AppAdminUraController);

    AppAdminUraController.$inject = ['$translate', '$state', 'cacheService', 'toastr', 'generalClientService', 'lodash', '$uibModal'];


    /** @ngInject */
    function AppAdminUraController($translate, $state, cacheService, toastr, generalClientService, lodash, $uibModal) {

        /* jshint validthis: true */
        var vm = this;
        vm.client = null;
        vm.user = null;
        vm.filter_type = 'uraName';
        ...
        function getUra() {
            generalClientService.getListClientUraByClientId(vm.client._id)
                .then(function (result) {
                    ...
                    vm.uraList = result;
                    vm.uraList_copy = angular.copy(result);
                });
        }
        ...
        function openForm(_item) {
            $state.go('admin_ura_form', {ura: _item, state_back: 'admin_ura'});
        }

        //modal de excluir
        function deleteUra(_item) {
            var modalInstance = $uibModal.open({
                ...
                resolve:      {
                    data: function () {
                        return {
                            message: "Deseja realmente excluir essa Ura?",
                            title:   "Excluir Ura"
                        }
                    }
                }
            ...
            modalInstance.result
                .then(function (result) {
                    if (result) {
                        generalClientService.removeClientUra(vm.client._id, _item.branch)
                            .then(function (result) {
                                if(result.length >= 1){
                                    vm.uraList = result;
                                    vm.uraList_copy = angular.copy(result);
        ...
        function executeFilter() {
            if (vm.filter_type !== null && vm.filter_type.length > 0) {
                if ( vm.filter_value !== undefined && vm.filter_value !== null ) {
                    var data_filtered = [];
                    vm.uraList = angular.copy(vm.uraList_copy);

                    switch (vm.filter_type) {

                        case 'queueNumber': {
                            data_filtered = lodash.filter(vm.uraList, function (reg) {
        ...
        function clearFilter() {
            vm.uraList = angular.copy(vm.branchList_copy);
            vm.filter_value = undefined;
        }
