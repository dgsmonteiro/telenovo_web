...
(function () {
    ...
    angular
        ...
        .controller('AppAdminConferenceController', AppAdminConferenceController);

    AppAdminConferenceController.$inject = ['$translate', '$state', 'cacheService', 'toastr', 'conferenceClientService', 'lodash', '$uibModal'];


    /** @ngInject */
    function AppAdminConferenceController($translate, $state, cacheService, toastr, conferenceClientService, lodash, $uibModal) {
        ...
        vm.filter_type = 'conferenceNumber';
        vm.executeFilter = executeFilter;
        vm.clearFilter = clearFilter;
        vm.getConference = getConference;
        vm.openForm = openForm;
        vm.deleteConference = deleteConference;
        ...
        function getConference() {
            conferenceClientService.getListConferenceByClientId(vm.client._id)
                .then(function (result) {
                    result = lodash.orderBy(result, ['number'], ['asc'])
                    vm.conferenceList = result;
                    vm.conferenceList_copy = angular.copy(result);
                });
        }
        ...
        function openForm(_item) {
            $state.go('admin_conference_form', {conference: _item, state_back: 'admin_conference'});
        }

        //modal de excluir ramal
        function deleteConference(_item) {
            var modalInstance = $uibModal.open({
                ...
                resolve:      {
                    data: function () {
                        return {
                            message: "Deseja realmente excluir essa Conferência?",
            ...
            modalInstance.result
                .then(function (result) {
                    if (result) {
                        conferenceClientService.removeClientConference(vm.client._id, _item.branch)
                            .then(function (result) {
                                if(result.length >= 1){
                                    vm.branchList = result;
                                    vm.branchList_copy = angular.copy(result);
                                    toastr.success(null, "Conferência excluida com sucesso", {"timeOut": "8000"});
                                }
        ...
        function executeFilter() {
            if (vm.filter_type !== null && vm.filter_type.length > 0) {
                if ( vm.filter_value !== undefined && vm.filter_value !== null ) {
                    ...
                    toastr.warning(null, $translate.instant('admin.settings.conference.filter_undefined'), {"timeOut": "8000"});
                }
            } else {
                toastr.warning(null, $translate.instant('admin.settings.conference.filter_undefined'), {"timeOut": "8000"});
            }
        ...
        function clearFilter() {
            vm.conferenceList = angular.copy(vm.branchList_copy);
            vm.filter_value = undefined;
        }
